package beginner;

import com.sandwich.koan.Koan;

import static com.sandwich.koan.constant.KoanConstants.__;
import static com.sandwich.util.Assert.assertEquals;

public class AboutBitwiseOperators {

    @Koan
    public void fullAnd() {
        int i = 1;
        if (true & (++i < 8)) i = i + 1;
        // the "true &" acts as a logical, non-short-circuited, AND operation
        assertEquals(i, 3);
    }

    @Koan
    public void shortCircuitAnd() {
        int i = 1;
        if (true && (i < -28)) i = i + 1;
        // the "true &" is logical, but in now a short-circuited, AND operation
        assertEquals(i, 1);
    }

    @Koan
    public void aboutXOR() {
        int i = 1;
        int a = 6;
        if ((a < 9) ^ false) i = i + 1;
        // true ^ false = true.  Thus i = i + 1 is executed
        assertEquals(i, 2);
    }

    @Koan
    public void dontMistakeEqualsForEqualsEquals() {
        int i = 1;
        boolean a = false;
        if (true == a) i++;
        assertEquals(a, false);
        assertEquals(i, 1);
        // How could you write the condition 'with a twist' to avoid this trap?
        // See above, swap so true == a.  true = a results in a compiler error, rather
        // than just a warning
    }

    @Koan
    public void aboutBitShiftingRightShift() {
        int rightShift = 8;
        rightShift = rightShift >> 1;
        assertEquals(rightShift, 4);
    }

    @Koan
    public void aboutBitShiftingLeftShift() {
        int leftShift = 0x80000000; // Is this number positive or negative?
        leftShift = leftShift << 1;
        // << shifts the 1 off the end, yielding 0
        assertEquals(leftShift, 0);
    }

    @Koan
    public void aboutBitShiftingRightUnsigned() {
        int rightShiftNegativeStaysNegative = 0x80000000;
        rightShiftNegativeStaysNegative = rightShiftNegativeStaysNegative >> 4;
        // >> fills with whatever the sign bit of 0x80000000 is (1)
        assertEquals(rightShiftNegativeStaysNegative, 0xF8000000);
        int unsignedRightShift = 0x80000000; // always fills with 0
        // >>> fills with 0s regardless of sign bit
        unsignedRightShift >>>= 4; // Just like +=
        assertEquals(unsignedRightShift, 0x08000000);
    }
}
